...
3. 浏览器打开 `http://localhost:53046/` 进入登录页面
4. 输入用户名和密码登录——**用户名密码在 Excel 表格中设定**，非硬编码
5. 登录后访问 **`http://127.0.0.1:53046/统计`** ——统计页面展示所有可用 API 的实际调用链接、示例请求和网关实时状态。**一定要打开这个页面才能看到 API 怎么用**
